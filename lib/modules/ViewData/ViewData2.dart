...
// Function to retrieve data about hosts
  ...
  @override
  void initState() {
    _getHostsFuture = getHosts(zabbixUrl: widget.zabbixUrl);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: RefreshIndicator(
        ...
        child: FutureBuilder(
            future: getHosts(zabbixUrl: widget.zabbixUrl),
            builder: (BuildContext context, AsyncSnapshot snapshot) {
              if (snapshot.connectionState == ConnectionState.waiting) {
                return Center(child: CircularProgressIndicator());
              } else if (snapshot.hasData) {
                List<dynamic> hosts = snapshot.data;
                return ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    String interfaceid =
                        hosts[index]['interfaces'][0]['interfaceid'];
                    return Padding(
  ...
  late Future<List<dynamic>> _getHostsFuture;

  Future<void> _refreshData() async {
    setState(() {
      _getHostsFuture = getHosts(zabbixUrl: widget.zabbixUrl);
    });
    await _getHostsFuture;
  }
}
